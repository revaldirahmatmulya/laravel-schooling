                    ...
                    <div class="row">
                        <div class="col-md-8">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <form
                                        action="{{ route('perpustakaan.category.update', ['bookcategory' => $category->id]) }}"
                                        method="post">
                                        @csrf
                                        @method('put')
                                        <div class="form-group">
                                            <label for="name">Nama kategori buku</label>
                                            <input type="text" class="form-control" id="name" name="name"
                                                placeholder="Nama kategori buku"
                                                value="{{ old('name', $category->name) }}">
                                            @error('name') <span class="text-danger">{{ $message }}</span> @enderror
                                        </div>
                                        <button class="btn btn-primary btn-rounded mt-2" type="submit">Edit Kategori
                                        </button>
