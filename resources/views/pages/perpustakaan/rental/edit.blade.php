...
@section('title', 'Edit Data Peminjam Buku')
                        ...
                        <div class="col-md-8">
                            <div class="card">
                                <div class="card-header card-info">
                                    <div class="card-title">Form Edit Data Peminjam Buku</div>
                                </div>
                                <div class="card-body">
                                    <form action="{{ route('perpustakaan.rental.update', ['rental' => $rentals]) }}"
                                        method="post">
                                        @csrf
                                        @method('put')
                                        <div class="form-group col-12">
                                            ...
                                            <select class="form-control" name="title" id="title" style="width: 100%"
                                                ...
                                                <option value="{{ $book->id }}"
                                                    {{ old('book', $rentals->book_id) == $book->id ? "selected" : "" }}>
                                                    {{ $book->title }}</option>
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="form-group col-12">
                                            ...
                                            <div class="row mx-1">
                                                <select class="form-control" name="student"
                                                    id="js-example-basic-multiple" style="width: 100%"
                                                    multiple="multiple" value="{{ old('student') }}">
                                                    @foreach ($students as $student)
                                                    <option value="{{ $student->id }}"
                                                        {{ old('student', $rentals->student_id) == $student->id ? "selected" : "" }}>
                                        ...
                                        <div class="form-group col-12">
                                            ...
                                            <select class="form-control status" name="status" id="status"
                                                style="width: 100%" value="{{ old('status') }}">
                                                @if ($rentals->status == 'completed')
                                                <option value="completed" selected>Completed</option>
                                                <option value="ongoing">Ongoing</option>
                                                @elseif($rentals->status == 'ongoing')
                                        ...
                                        <div class="form-group col-12">
                                            <label for="tanggal">Dipinjam Pada</label>
                                            <input type="date" class="form-control" id="created_at" name="created_at"
                                                value="{{ old('created_at', date('Y-m-d', strtotime( $rentals->created_at))) }}"
                                                disabled>
                                            <input type="date" class="form-control" id="created_at" name="created_at"
                                                value="{{ old('created_at', date('Y-m-d', strtotime( $rentals->created_at))) }}"
                                                hidden>
                                        ...
                                        <div class="form-group col-12">
                                            ...
                                            <input type="date" class="form-control" id="return_date" name="return_date"
                                                value="{{ old('return_date', $rentals->return_date) }}">
                                            @error('return_date') <span class="text-danger">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        <div class="d-flex col-3 mb-4 justify-content-start">
                                            <button class="btn btn-primary btn-rounded">Edit Data Peminjam</button>
