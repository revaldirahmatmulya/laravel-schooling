						...
						<div class="col-md-10   ">
							<div class="card">
                                <div class="card-header card-info">
                                    <div class="card-title">Form tambah Pasien baru</div>
                                </div>
								<div class="card-body">
                                <form action="{{ route('uks.pasien.store') }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <div class="form-group">
                                        <label for="name">Nama Pasien</label>
                                        <input type="text" class="form-control" id="name" name="name" placeholder="Nama Pasien" value="{{ old('name') }}">
                                        @error('name') <span class="text-danger">{{ $message }}</span> @enderror
                                    </div>  
                                    <div class="form-group">
                                        <label for="keterangan_pasien">Keterangan Pasien</label>
                                        <input type="text" class="form-control" id="keterangan_pasien" name="keterangan_pasien" placeholder="Keterangan Pasien" value="{{ old('keterangan_pasien') }}">
                                        @error('keterangan_pasien') <span class="text-danger">{{ $message }}</span> @enderror
                                    </div> 
                                    <div class="form-group">
                                        <label for="keluhan">Keluhan</label>
                                        <textarea name="keluhan" id="keluhan" class="form-control" cols="3" rows="3">{{ old('keluhan') }}</textarea>
                                        @error('keluhan') <span class="text-danger">{{ $message }}</span> @enderror
                                    </div>  
                                    <div class="form-group">
                                        <label for="penanganan">Penanganan</label>
                                        <textarea name="penanganan" id="penanganan" class="form-control" cols="3" rows="3">{{ old('penanganan') }}</textarea>
                                    ...
                                    <div class="mt-3 float-right">
                                        ...
                                        <button class="btn btn-primary btn-rounded">Tambah Pasien</button>
                                    </div>
...
@section('js')
<script>
    var old = {{ $old_obat != null ? count($old_obat) : '2' }}
