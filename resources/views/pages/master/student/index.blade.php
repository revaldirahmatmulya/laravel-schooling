...
@section('js')
    <script>
				...
				columns: [
					...
					},
					{
						...
						render: function(data, type, row){
							...
							return '\
							<a href="'+url_show+'" class="btn btn-xs btn-primary my-1">Details</a>\
							<a href="'+url_edit+'" class="btn btn-xs btn-warning my-1">Edit</a>\
							<button class="btn btn-xs btn-danger my-1" id="delete" onclick="delete_user('+row.user["id"]+', \'user/admin\')">Delete</button>';
						}
					},
				]
			});
		...
		function delete_user(id,url1) {
			var url = "{!! url('master/user/admin'); !!}";
			var _token = "{{ csrf_token() }}";
			Swal.fire({
